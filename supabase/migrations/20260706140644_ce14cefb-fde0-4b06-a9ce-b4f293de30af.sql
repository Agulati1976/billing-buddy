...
BEGIN
  ...
  IF is_tracked THEN
    IF NEW.batch_id IS NULL AND inv_type IN ('sale', 'purchase_return') THEN
      RAISE EXCEPTION 'Pick a batch for % before saving. Batch-tracked stock cannot be sold without a batch.', COALESCE(item_name, NEW.item_name, 'this item')
        USING ERRCODE = 'P0001';
    END IF;
    RETURN NEW;
  END IF;
